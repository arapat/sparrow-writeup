...
\twocolumn[
\sysmltitle{Accelerated learning through early stopping and re-weighting}
...
\begin{abstract}
In this paper we present several methods for reducing the I/O
and communication requirements of parallelized tree boosting.

The inner loop of the tree boosting algorithm is a search for
splitting rules. This search is guided by estimates of the purity
function. Traditionally, these estimates are based on {\em all
  available data}. Our first observation is that, when the training
data is large, it is sometimes sufficent to compute an estimate based
on just a small fraction of the data. This results in a reduce memory
footprint and faster execution.

Our second obsrvation regards the parallelization of the search
process. For this case we propose an asynchronous communication we
call ``Tell Me Something New'' or \tmsn\. According to this protocol
the first computer to find a good rule interrupts the search of the
other machines. We show that this protocol can yield significant
saving in time when compared to bulk-synchronous parallelism.  It is
also highly robust to laggards and to computer failure.

We implemented our methods applied the implementation to the
